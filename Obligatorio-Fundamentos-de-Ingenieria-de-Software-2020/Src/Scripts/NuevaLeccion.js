...
function cancelar() {
  location.href = '../html/NuevaLeccion.html';
}
//onclick
...
function generaTabla() { //Partes del codigo obtenido de https://developer.mozilla.org/es/docs/Trazado_de_una_tabla_HTML_mediante_JavaScript_y_la_Interface_DOM
...
function crearLeccion() {
  ...
  var unaLeccion = new Leccion(cancion, interprete, dificultad, tempo, ObjTablatura);
  return unaLeccion;
}
...
function validarCamposRequeridos() {
  ...
  if (validarCamposRequeridosBack(cancion, interprete, tempo) == false) {
    alert('Corrobore que todos los campos estén completos');
    return false;
  } else {
    return true;
  }
...
function validacionCaracterAcorde(key) {
  //Valida que las teclas son: b,h,p,h,s,49,50,51,52,53,54,55,56,57,48
  if (!((key > 47 && key <= 57) || (key > 95 && key <= 105) || (key == 8) || (key == 66)
    || (key == 88) || (key == 80) || (key == 72) || (key == 83) || (key == 118))) {
    return false
  } else {
    return true
  }
